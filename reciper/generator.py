...
def generate_files(
    ...
    try:
        environment_content = generate_environment_yml_from_specs(package_specs)
...
def generate_environment_yml_from_specs(package_specs: list[str]) -> str:
    """
    ...
    if include_apt and python_files:
        try:
            ...
            if all_command_calls:
                apt_pkg_objects = map_commands_to_apt_packages(all_command_calls)
                # Extract package names
                apt_packages = [pkg.package_name for pkg in apt_pkg_objects]
                
        except ImportError:
            # Command detector not available, skip apt detection
            pass
    
    # Generate Dockerfile with apt packages
    return generate_dockerfile_with_apt(
